...
(function ($, ours) {
     ...
     *       afterInit: ours._noop,//初始化完成
     *       beforeSelect: ours._noop,//从左到右的移过去前拦截 返回false将不会移动过去
     *       afterSelect: ours._noop,//从左到右的移过去后拦截
     *       callbackOk: ours._noop,//确定回调 返回false将不会关闭窗口
     *       callbackCancel: ours._noop,//取消回调
     *       }
     * @param isClient isClient
     */
    ours._selectPerson = function (options, isClient) {
        var _options = {
        ...
        switch (_options.showType || 0) {
            case 0:
                chooseType.push(ours.ORG_TYPE_ENUM.contact);
                break;
            case 1:
                chooseType.push(ours.ORG_TYPE_ENUM.team);
                break;
            case 2:
                chooseType.push(ours.ORG_TYPE_ENUM.contact);
                chooseType.push(ours.ORG_TYPE_ENUM.team);
                break;
            case 3:
                chooseType.push(ours.ORG_TYPE_ENUM.person);
                _options.allowCompany = false;
                break;
            case 4:
                chooseType.push(ours.ORG_TYPE_ENUM.department);
                chooseType.push(ours.ORG_TYPE_ENUM.relativeRole);
                chooseType.push(ours.ORG_TYPE_ENUM.level);
                chooseType.push(ours.ORG_TYPE_ENUM.post);
                chooseType.push(ours.ORG_TYPE_ENUM.role);
            ...
            case 6://这里代表自定义chooseType
                ...
                for(var i = 0,len = tempChooseType.length;i < len;i++){
                    ...
                    if(typeEnumKey !== 'all' && typeEnumKey !== 'company' && typeEnumKey !== 'group' && typeEnumKey !== 'orgTeam'){
                        if(ours.ORG_TYPE_ENUM[typeEnumKey]){
                            chooseType.push(typeEnumKey);
                        }
                    }
                }
                if(!chooseType || chooseType.length <= 0){
                    ours.getTop().ours.alert("请传入选择类型");
                    return;
                }
                break;
            default:
        ...
        if (_options.showType && _options.showType >= 3) {
            ...
            if (isClient) {
                ours.$.ctrl.dialog.SelectPersonDialog = new selectPerson4AppClz(_options);
                selectPerson4App(_options);
                return ours.$.ctrl.dialog.SelectPersonDialog;
            }
        }
        _options.isFlow = (_options.isFlow + "" !== "false");

        if(!selectPersonMainFrameHTML){
            selectPersonMainFrameHTML = ours.loadUrl(ours_url.selectPerson);
        }

        var SelectPersonDialog = ours.showHTMLDialog4Div({  //创建唯一SelectPerson弹出框对象
        ...
        ours.$.ctrl.dialog.SelectPersonDialog = SelectPersonDialog;

        ours.SelectPerson.init(SelectPersonDialog.attr("selectPersonOptions"));

        return SelectPersonDialog;
    };

    ours.selectPerson = function (options) {
        return ours._selectPerson(options, false);
    };

    ours.selectPerson4Client = function (options) {
        return ours._selectPerson(options, true);
    };

    ours.selectPerson.TypeEnum = {
        COMMON: 0,//普通选人
        FLOW: 1//流程选人
    };

    var selectPersonVo = function (obj, type) {
        if (ours.appType.qing || ours.appType.enterpriseKdweibo) {
            this.name = obj.name;
            this.id = obj.openId;
            //this.icon = obj.avatarUrl;
            this.typeFlag = ours.ORG_TYPE_ENUM.person;
        } else if (ours.appType.enterpriseWechat) {
            this.name = obj.name;
            this.id = obj.id;
            this.typeFlag = type;
        } else if (ours.appType.dingtalk) {
            this.name = obj.name;
            this.id = (type == ours.ORG_TYPE_ENUM.person ? obj.emplId : obj.id);
            this.typeFlag = type;
            //this.icon = (type == ours.ORG_TYPE_ENUM.person ? obj.avatar : "");
        } else if (ours.appType.qyquan) { /*企业圈、M3*/
            this.name = obj.name;
            this.id = obj.accNbr;
            this.typeFlag = ours.ORG_TYPE_ENUM.person;
        } else if (ours.appType.m3) {
            this.name = obj.name;
            this.id = obj.id;
            this.typeFlag = ours.ORG_TYPE_ENUM.person;
        }
     ...
     * @param type
     */
    var appSpData2OursVo = function (items, type) {
        if (!items) return [];
        var currentClientContext = ours.getCurrentClientContext();
    ...
    ours.appSpData2OursVo = appSpData2OursVo;
    ...
    var checkSelectPersons = function (result, filterSelf, callback) {
        if (result && result.length > 0) {
            ...
            //} else {
            var __appName = ours.appType.qing ? "云表单" : "云积木";
            param['users'] = result;

            ours.postData(ours_url.selectPerson_userId2AccountId, param, function (res) {
                res = ours.parseJson(res);
                if (res.success) {
                    var data = res.msg;
                    data = ours.parseJson(data);
                    ...
                    if (notInsideList.length > 0) {
                        ...
                        for (var i = 0, len = notInsideList.length; i < len; i++) {
                            ...
                            if (notVo.typeFlag === ours.ORG_TYPE_ENUM.person) {
                ...
                ours.alert(ours.parseString(error));
            });
            //}
        } else {
    ...
    var accountId2userId = function (result, callback) {
        if (result && result.length > 0) {
            var param = {};
            param['users'] = result;
            ours.postData(ours_url.selectPerson_accountId2UserId, param, function (res) {
                res = ours.parseJson(res);
                if (res.success) {
                    var data = res.msg;
                    data = ours.parseJson(data);
    ...
    var selectPerson4App = function (options) {
        ...
        if (selectTypes && selectTypes.length > 0) {
            if (selectTypes.indexOf(ours.ORG_TYPE_ENUM.person) < 0) {
                canSelectUser = false;
            }
            if (selectTypes.indexOf(ours.ORG_TYPE_ENUM.department) < 0) {
                canSelectDep = false;
            }
        }
            ...
            try {
                if (fillbackData && fillbackData.length > 0) {
                    ...
                    for (var i = 0, len = fillbackData.length; i < len; i++) {
                        ...
                        if (item.typeFlag == ours.ORG_TYPE_ENUM.person) {
                            selectedUserIds.push(id);
                        } else if (item.typeFlag == ours.ORG_TYPE_ENUM.department) {
                            selectedDepIds.push(id);
                        }else if(item.typeFlag == ours.ORG_TYPE_ENUM.company || item.typeFlag == ours.ORG_TYPE_ENUM.all){
                            selectedDepIds.push(id);
                        }
                    }
                }
            } catch (e) {
                ours.alert("回调数据出错" + ours.parseString(e));
            }
            var currentClientContext = ours.getCurrentClientContext();
            if (currentClientContext && currentClientContext.selectPersonConfig) {
                var selectPersonConfig = currentClientContext.selectPersonConfig;
...
(function (win) {
    /*******************************依赖的Js类 start***********************************************************/
    var ctrl = ours.$.ctrl;
    var Control = ctrl.oursformcontrol;
    /*******************************依赖的Js类 end************************************************************/
    ...
    SelectPerson.FullName = "ours.$.ctrl.selectperson";//设置当前类全名 静态变量
    /**
     * 定义 html模板,
    ...
    SelectPerson.templateHtml[0] =
        '<input type="hidden" validate="{{validate}}" targetHighBorderEl="ours.getNS().$(\'#ours_selectperson_{{id}}\')" id="{{id}}" name="{{name}}" value="{{value}}" />\
        <div id="ours_selectperson_{{id}}" class="{{isMulti ? \'multi-\':\'\'}}personarea" onclick="ours.getByOursId({{oursId}}).showSelectPerson(this);">{{ours.getByOursId(oursId).getDisplay4readOnly();}}</div>\
    ...
    SelectPerson.templateHtml4readOnly[0] = '{{ours.getByOursId(oursId).getDisplay4readOnly();}}';
    ...
    var init = function () {
        ...
        var otherAttrs = ours.parseJson(this.attr("otherAttrs") || "{}");
    ...
    var showSelectPerson = function (obj) {
        var self = this,
            ...
            data = ours.parseJson(this.attr("data") || '[]'),
        ...
        if (sExtend) {
            controlOptions["extend"] = ours.parseJson(sExtend);
        }
        ...
        //     _options['rangeId'] = ours.RANGE_TYPE.currentDept;
        // }

        var rangeType = otherAttrs.rangeType;
        if (rangeType === "company") {//如果全部选人
            _options["onlyRange"] = false;
            _options["rangeId"] = ours.RANGE_TYPE.currentDept;//不限定范围，但是展开当前部门
            ...
            var data4DB = null;
            var item = null;
            var selectDeptControl = ours.getById(otherAttrs.rangeId);
            if (selectDeptControl) {//源控件不存在页面上，说明当前表单是明细表，且不是原样表单
                ...
                if (deptId) {
                    ...
                    if(item && item.typeFlag !== ours.ORG_TYPE_ENUM.company){
                        flag = true;
                        _options["rangeId"] = selectDeptControl.getValue();
                    }
                }
            } else {
                var tableOursId = self.attr("tableOursId");
                var table = ours.getByOursId(tableOursId);
                var $tr = $(self.getEl()).closest(".ours-table-grid-row");
                if(table && $tr.length > 0){
                    ...
                    if(row[otherAttrs.rangeId]){//选部门控件是明细表的
                        selectDeptControl = ours.getByOursId(row[otherAttrs.rangeId].controlOursId);
        ...
        ours.selectPerson($.extend(true, {
            ...
            callbackOk: function (result) {
                ...
                ours.validate($(self.getEl()).find('#' + self.attr('id'))[0]);
            }
        ...
        //    callbackOk: function (result) {
        //        var data = result.data;
        //        self.attr("value", ours.parseString(data));
        //        self.attr("data", data);
        //        self.render();
        //        ours.validate($(self.getEl()).find('#' + self.attr('id'))[0])
        //    }
    ...
    var getDisplay4readOnly = function () {
        ...
        for (var i = 0, len = d.length; i < len; i++) {
            if (v.indexOf(d[i].id) > -1) {
                showTextArray.push(ours.escapeStringToHTML(d[i].name));
            }
        }
...
(function ($, doc, ours) {
     ...
     */
    var ServiceApi = {
        getPerson: ours_url.selectPerson_getPerson,//获取人接口
        searchByName: ours_url.selectPerson_searchByName,//获取组织对象根据name（搜索接口）
        getOrg: ours_url.selectPerson_getOrg,//获取组织机构(id不传代表，type:company获取所有部门，传了id，type：department代表获取子部门)
        getCustomRole: ours_url.selectPerson_getCustomRole,
        userId2AccountId: ours_url.selectPerson_userId2AccountId,
        accountId2UserId: ours_url.selectPerson_accountId2UserId
    };
    ...
    SelectPersonBiz.userId2AccountId = function (result, filterSelf, callback) {
        if (result && result.length > 0) {
            ...
            var __appName = ours.appType.qing ? "云表单" : "云积木";
            ours.postData(getSeverURL("userId2AccountId"), param, function (res) {
                res = ours.parseJson(res);
                if (res.success) {
                    var data = res.msg;
                    data = ours.parseJson(data);
                    ...
                    if (notInsideList.length > 0) {
                        ...
                        ours.alert(notInsideNameStr + "还没有注册" + __appName + "哦！", function () {
                            callback && callback({data: data.checkedVoList});
                        });
                    } else {
                        callback && callback({data: data.checkedVoList});
    ...
    SelectPersonBiz.accountId2userId = function (result, callback) {
        if (result && result.length > 0) {
            var param = {};
            param['users'] = result;
            ours.postData(getSeverURL("accountId2UserId"), param, function (res) {
                res = ours.parseJson(res);
                if (res.success) {
                    var data = res.msg;
                    data = ours.parseJson(data);
    ...
    SelectPersonBiz.getPersonByOrgObj = function (orgObj, callback, otherParam) {
        ...
        if (userCache && SelectPersonBiz.personMap && SelectPersonBiz.personMap[orgObj.id + '_' + orgObj.typeFlag]) {
            ...
            ours.getData(getSeverURL("getPerson", param), {}, function (res) {
                if (res.success) {
                    var result = res.msg;
                    result = ours.parseJson(result);
                    var name = null;
                    var display = null;
                    var post = null;
    ...
    SelectPersonBiz.getOrg = function (type, id, callback, otherParam, isClickDepartment) {
        ...
        if (type === ours.ORG_TYPE_ENUM.company || type === ours.ORG_TYPE_ENUM.department) {
            if (SelectPersonBiz.departmentMap) {
                if (type === ours.ORG_TYPE_ENUM.company || isClickDepartment) {
                    callback && callback(SelectPersonBiz.firstDepartMent);
                } else if (type === ours.ORG_TYPE_ENUM.department) {
                ...
                ours.getData(getSeverURL("getOrg", param), {}, function (res) {
                    if (res.success) {
                        var result = res.msg;
                        result = ours.parseJson(result);
                        var newArray = [];
    ...
    SelectPersonBiz.getChildDepartmentByOrg = function (org) {
        if (org.typeFlag === ours.ORG_TYPE_ENUM.department) {
            if (SelectPersonBiz.departmentMap) {
    ...
    SelectPersonBiz.getChildOrgByOrg = function (org, callback, noNeedPerson) {
        if (org.typeFlag === ours.ORG_TYPE_ENUM.department || org.typeFlag === ours.ORG_TYPE_ENUM.company) {
            SelectPersonBiz.getOrg(ours.ORG_TYPE_ENUM.department, org.id, function (_departResult) {
                var departResult = [];
        ...
        for (var key in relativeRoles) {
            if (relativeRoles.hasOwnProperty(key)) {
                _relativeRoleArray.push({
                    ...
                    typeFlag: ours.ORG_TYPE_ENUM.relativeRole
                });
            }
        }
    ...
    SelectPersonBiz.searchByName4Org = function (type, keyword, id, callback) {
        ...
        ours.getData(getSeverURL("searchByName", param), {}, function (res) {
            if (res.success) {
                var result = res.msg;
                result = ours.parseJson(result);
    ...
    SelectPersonBiz.getDataByExtendTab = function (tab, callback) {
        if (tab.url && typeof tab.url === "string") {
            ours.getData(tab.url, {}, function (res) {
                if (res.success) {
                    var result = res.msg;
                    result = ours.parseJson(result);
    ...
    ours.SelectPersonBiz = SelectPersonBiz;
})(jQuery, document, ours);;
/**
 * Created by yangh on 2016/4/6.
 *
 */
(function ($, doc, ours) {
    ...
    //onclick="ours.SelectPerson.optionChange(this,\'{{item.typeFlag}}\')"
    //<span>（兼）积木云产品与运营组</span>\
    templateHTML['spSelectTpl'] =
        '{{if data.length > 0}}\
            {{each data as item index}}\
                {{if item.typeFlag != "person" && item.typeFlag != "contact" && item.typeFlag != "relativeRole" && ours.ORG_TYPE_ENUM[item.typeFlag]}}\
                    <div>\
                        <label class="selectcheckbox" for="sp_{{item.id}}">\
                            {{if selectPerson.checkSelectType(item)}}\
                                ...
                                <div class="selectperson-checkbox" itemId="{{item.id}}" id="allSelectedCheckbox">\
                                    <input id="sp_{{item.id}}" onchange="ours.SelectPerson.checkChange(this,{{ours.parseString(item)}})" type="checkbox" {{if selectPerson.isSelected(item)}}checked="checked"{{/if}} >\
                                    <i class="selected-icon"></i>\
                                </div>\
                                {{else}}\
                                <div class="selectperson-checkbox">\
                        ...
                        {{if item.typeFlag == "department" || item.typeFlag == "company"}}\
                            <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' link="#/department/spData/" >\
                                <span class="selectportrait depart-icon">&nbsp;</span>\
                                <span class="selectFlex">{{item.name}}</span>\
                                <i class="select-user-info"></i>\
                            </div>\
                        {{else}}\
                            <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' link="{{if item.typeFlag=="team"}}#/team/spData/{{else}}#/child/spData{{/if}}" >\
                                {{if item.typeFlag=="team"}}\
                                <span class="selectportrait team-icon">&nbsp;</span>\
                                {{else}}\
                                <span class="selectportrait userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                                {{/if}}\
                                <span class="selectFlex">{{item.name}}</span>\
                                <i class="select-user-info"></i>\
                            </div>\
                ...
                {{else}}\
                    <div>\
                        ...
                        <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' {{if selectPerson.checkSelectType(item)}}onclick="ours.SelectPerson.optionChange(this,\'{{item.typeFlag}}\')"{{/if}}>\
        ...
        templateHTML['spSelectWxAddressTpl'] =
        '{{if data.length > 0}}\
            {{each data as item index}}\
                {{if item.typeFlag != "person" && item.typeFlag != "contact" && item.typeFlag != "relativeRole" && ours.ORG_TYPE_ENUM[item.typeFlag]}}\
                    <div>\
                        ...
                        {{if item.typeFlag == "department" || item.typeFlag == "company"}}\
                            <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' link="#/department/spData/" >\
                                <span class="selectportrait depart-icon">&nbsp;</span>\
                                <span class="selectFlex"><ww-open-data type="departmentName" openid="{{item.name}}"></ww-open-data></span>\
                                <i class="select-user-info"></i>\
                            </div>\
                        {{else}}\
                            <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' link="{{if item.typeFlag=="team"}}#/team/spData/{{else}}#/child/spData{{/if}}" >\
                                ...
                                <span class="selectportrait userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                                {{/if}}\
                                <span class="selectFlex">000{{item.name}}</span>\
                                <i class="select-user-info"></i>\
                            </div>\
                        {{/if}}\
                    </div>\
                {{else}}\
                    <div>\
                        ...
                        <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' {{if selectPerson.checkSelectType(item)}}onclick="ours.SelectPerson.optionChange(this,\'{{item.typeFlag}}\')"{{/if}}>\
                            ...
                            <span class="selectportrait userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                            {{/if}}\
                            <span class="selectFlex"><ww-open-data type="userName" openid="{{item.name}}"></ww-open-data></span>\
                        </div>\
                    </div>\
                {{/if}}\
            {{/each}}\
        ...
        templateHTML['spLFWxAddressTpl'] = templateHTML['spLFWxAddress'] = templateHTML['spSelectWxAddressTpl']
        
        templateHTML['spLFTpl'] = templateHTML['spLT4ContactTpl'] = templateHTML['spSelectTpl'];
    
        templateHTML['spSelectedTpl'] =
            '{{each data as item index}}\
                <li id="sp_selected_{{item.id}}" onclick="ours.SelectPerson.removeItem({{ours.parseString(item)}})">\
                    {{if item.typeFlag=="team"}}\
                    ...
                    {{else if item.typeFlag=="relativeRole"}}\
                        <span class="userimg userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                    {{else}}\
                        <span class="userimg userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                    {{/if}}\
        ...
        templateHTML['spSelectedWxAddressTpl'] =
        '{{each data as item index}}\
            <li id="sp_selected_{{item.id}}" onclick="ours.SelectPerson.removeItem({{ours.parseString(item)}})">\
            {{item.typeFlag}}\
            {{item.typeFlag=="person" ? "1111" : "2222"}}\
                {{if item.typeFlag=="team"}}\
                    <span class="userimg team-icon">&nbsp;</span>\
                {{else if item.typeFlag=="department"}}\
                    <span class="userimg depart-icon">&nbsp;</span>\
                ...
                {{if item.typeFlag=="person"}}\
                    <span class="username"><ww-open-data type="userName" openid="{{item.name}}"></ww-open-data></span>\
                {{else}}\
                    {{if item.typeFlag=="department"}}\
                        <span class="username"><ww-open-data type="departmentName" openid="{{item.name}}"></ww-open-data></span>\
                    {{else}}\
                        <span class="username">{{item.name}}</span>\
                    {{/if}}\
                {{/if}}\
    ...
    templateHTML['spNavigationTpl'] =
        ...
        <span link="{{item.linkTo}}" spData="{{ours.parseString(item.spData)}}"><i>{{item.name}}</i></span>{{if data.length != 1}}&gt;{{/if}}\
        {{else}}\
        <span>{{item.name}}</span>\
        {{/if}}\
        {{/each}}\
    ...
    templateHTML['spNavigationWxAddressTpl'] =
        '{{each data as item index}}\
        {{if index != (data.length-1) || data.length == 1}}\
            {{if (item.linkTo.indexOf("department") != -1) && (item.linkTo.indexOf("spData") != -1)}}\
            <span link="{{item.linkTo}}" spData="{{ours.parseString(item.spData)}}"><i><ww-open-data type="departmentName" openid="{{item.name}}"></ww-open-data></span></i></span>{{if data.length != 1}}&gt;{{/if}}\
            {{else}}\
            <span link="{{item.linkTo}}" spData="{{ours.parseString(item.spData)}}"><i>{{item.name}}</i></span>{{if data.length != 1}}&gt;{{/if}}\
            {{/if}}\
        {{else}}\
            {{if (item.linkTo.indexOf("department") != -1) && (item.linkTo.indexOf("spData") != -1)}}\
            <span><ww-open-data type="departmentName" openid="{{item.name}}"></ww-open-data></span>\
            {{else}}\
            <span>{{item.name}}</span>\
            {{/if}}\
        {{/if}}\
    ...
    templateHTML['spOptionTpl'] =
        '{{if data.isFlow || data.isFlow=="true" || data.isFlow == true}}\
            <div>\
                <button onclick="ours.SelectPerson.click2Ok(0);">并发</button>\
            </div>\
            <div>\
                <button onclick="ours.SelectPerson.click2Ok(1);">串发</button>\
            </div>\
        {{else}}\
            <div>\
                <button onclick="ours.SelectPerson.click2Ok();">确认</button>\
            </div>\
        {{/if}}';
        ...
        '{{each data as item index}}\
             <div>\
                <label class="selectcheckbox" for="sp_s_{{item.id}}">\
                    <div class="selectperson-checkbox">\
                        <input id="sp_s_{{item.id}}" sp_search="true" onchange="ours.SelectPerson.checkChange(this,{{ours.parseString(item)}})" type="checkbox" {{if selectPerson.isSelected(item)}}checked="checked"{{/if}} >\
                        <i class="selected-icon"></i>\
                    </div>\
                </label>\
                <div toView="spR" class="selectInfo" spData=\'{{ours.parseString(item)}}\' onclick="ours.SelectPerson.optionChange(this,\'{{item.typeFlag}}\')" >\
                    <span class="selectportrait userIcon-bgColor-{{ours.SelectPerson.getIconBgNum(item)}}">{{item.name.substring(0,1)}}</span>\
                    <span class="selectFlex">{{item.name}}</span>\
                </div>\
            </div>\
    ...
    templateHTML['spSearchHistoryTpl'] =
        ...
        + templateHTML['spSearchSelectTpl'] +
        '<button onclick="ours.SelectPerson.clearSearchHistory()" class="clear-history">清除历史记录</button>\
        ';
    ...
    SelectPersonUI.render = function (key, data, noReplace) {
        var wxAddressKey = key
        // if (/MicroMessenger/i.test(navigator.userAgent)) {
            if (key == 'spSelect' || key =="spLF" || key == 'spSelected' || key == 'spNavigation') {
                wxAddressKey = wxAddressKey + "WxAddress"
            }
        // }
        var renderFunc = this.renders[wxAddressKey];
        if (!renderFunc) {
            renderFunc = template.compile(templateHTML[wxAddressKey + "Tpl"]);
    ...
    ours.SelectPersonUI = SelectPersonUI;
...
(function ($, doc, ours, Biz, UI) {
    ...
    SelectPerson.click2department = function () {
        ...
        var type = ours.ORG_TYPE_ENUM.company;
        var showType = self.options.showType;
        var id = null;
        var isOnlyRange = self.options.onlyRange + '' === 'true';
        if(isOnlyRange){//是否限定范围
            if (rangeId && rangeId !== ours.RANGE_TYPE.company && rangeId !== ours.RANGE_TYPE.currentDept) {
                type = ours.ORG_TYPE_ENUM.department;
                id = rangeId;
            } else if (rangeId === ours.RANGE_TYPE.currentDept) {
                type = ours.ORG_TYPE_ENUM.department;
        ...
        Biz.getOrg(type, id, function (result) {
            if (result && result.length > 0 && type === ours.ORG_TYPE_ENUM.company) {
            ...
            if (!isOnlyRange && id !== null && id.length > 0 && self.isFristClick2department) {
                ...
                Biz.getChildOrgByOrg({
                    id:id,
                    typeFlag:ours.ORG_TYPE_ENUM.department
                }, function (res) {
                    UI.render("spLF", res);
                }, showType + '' === '4');
            } else {
    ...
    SelectPerson.click2role = function(){
        this.setCurrentSelectType(SP_SEARCH_TYP_ENUM.ORG_PERSON);
        Biz.getOrg(ours.ORG_TYPE_ENUM.role, null, function (result) {
            UI.render("spLF", result);
        }, null, true);
    };

    /**
     * ours 帐号ID 转为第三方 userID
     * @param sourceData
     * @param callback
     */
    SelectPerson.accountId2UserId = function (sourceData, callback) {
        //扫描登陆转
        var showType = this.options.showType;
        if (showType >= 3 && (sourceData[0].typeFlag !== ours.ORG_TYPE_ENUM.all)) {//如果为组织机构选人，需要走检查转换ID
            callback && callback(sourceData);
            //FIXME  用我们自己的选人不需要将oursId 转成 第三方的ID
            // Biz.accountId2userId(sourceData, function (result) {
            //     callback && callback(result);
            // });
        } else {
            if (sourceData[0].typeFlag === ours.ORG_TYPE_ENUM.all) {
    ...
    SelectPerson.searchHistory = {
        add: function (item) {
            ...
            //}
            //historyData = ours.parseString(historyData);
            //ours.storage.save(SP_HISTORY_KEY, historyData);

        },
        clear: function () {
            //ours.storage.save(SP_HISTORY_KEY, "[]");
        },
        get: function () {
            //var historyData = ours.storage.get(SP_HISTORY_KEY);
            //historyData = ours.parseJson(historyData);
    ...
    SelectPerson.showSearchLayer = function () {
        ...
        if (!byType || ours.ORG_TYPE_ENUM[byType]) {
            if (showType >= 3) {
                this.setCurrentSelectType(SP_SEARCH_TYP_ENUM.ORG_PERSON);
            }
        }
    ...
    SelectPerson.checkSelectType = function (item) {
        ...
        if (ours.ORG_TYPE_ENUM[item.typeFlag]) {
            if (ours.ORG_TYPE_ENUM.company === item.typeFlag) {
    ...
    SelectPerson.checkSelected = function (item) {
        ...
        if (isMulti) {
            if (maxSize > 1 && mapLength >= maxSize) {
                if (showType >= 3) {
                    if (showType + "" === "4") {
                        ours.alert("您只能选择" + maxSize + "个部门");
                    } else {
                        ours.alert("您只能选择" + maxSize + "个");
                    }
                }
                return false;
            }
    ...
    SelectPerson.renderSelectedView = function (obj, isDelete) {
        if (isDelete+'' === 'true') {
            ...
            for (var i = 0, len = obj.length; i < len; i++) {
                ...
                if(obj[i].typeFlag === ours.ORG_TYPE_ENUM.person){
        ...
        if (ours.getObjectLength(SelectPerson.selectMap) > 0) {
            UI.spDeleteTips.show();
        } else {
            UI.spDeleteTips.hide();
        }
        // UI.spSelectedView.css("width", (ours.getObjectLength(SelectPerson.selectMap) * 0.7 ) + "rem");
    ...
    SelectPerson.checkChange = function (obj, item) {
        ...
        if (obj.checked) {
            ...
            if (self.checkSelected(item)) {
                if (item.typeFlag === ours.ORG_TYPE_ENUM.department || item.typeFlag === ours.ORG_TYPE_ENUM.company) {//如果是部门
                    if (item.children || item.typeFlag === ours.ORG_TYPE_ENUM.company) {
                        if (isAll && (item.level+'' ==='0')) {
                            item = {
                                id: item.id,
                                name: "全体人员",
                                typeFlag: ours.ORG_TYPE_ENUM.all
                            };
                        ...
                        if (includeChildType + '' === '0') {
                            var addItems = [];
                            ours.getTop().ours.confirmDialog("此部门下包含子部门，是否选中子部门", function () {
    ...
    SelectPerson.optionChange = function (obj, type) {
        ...
        switch (type) {
            ...
            case ours.ORG_TYPE_ENUM.role://角色
            case ours.ORG_TYPE_ENUM.level://职务级别
    ...
    SelectPerson.getSelectedData = function () {
        ...
        for (key in this.selectMap) {
            ...
            if (selectItem) {
                if (selectItem.typeFlag === ours.ORG_TYPE_ENUM.all) {
                    return [selectItem];
                }
        ...
        switch (self.currentSelectType) {
            case SP_SEARCH_TYP_ENUM.ORG_PERSON:
                if (showType + '' === '4') {
                    ...
                    Biz.searchByName4Org(ours.ORG_TYPE_ENUM.department, keyword, id, function (result) {
                        UI.render('spSearchSelect', result);
                    });
                }
                break;
            default :
                if (!ours.ORG_TYPE_ENUM[self.currentSelectType]) {
    ...
    SelectPerson.close = function () {
        ours.$.ctrl.dialog.SelectPersonDialog.close();
    ...
    ours.SelectPerson = SelectPerson;
})
(jQuery, document, ours, ours.SelectPersonBiz, ours.SelectPersonUI);